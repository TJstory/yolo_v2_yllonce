...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    printf("%s\n", base);//.cfg采用的是yolo2
    float avg_loss = -1;
    network *nets = calloc(ngpus, sizeof(network));//分配网络动态地址
    ...
    for(i = 0; i < ngpus; ++i){
        srand(seed);
#ifdef GPU //宏定义中有GPU则编译以下代码
        cuda_set_device(gpus[i]);
#endif
        nets[i] = parse_network_cfg(cfgfile);//重点分析的函数！！！
        if(weightfile){
    ...
    args.h = net.h;
    args.paths = paths;
    args.n = imgs;//一轮训练的图片数
    args.m = plist->size;//待训练的图片数
...
{
    char *prefix = find_char_arg(argc, argv, "-prefix", 0);//*char 专门用于指以'\0'为结束的字符串
    float thresh = find_float_arg(argc, argv, "-thresh", .24);
    float hier_thresh = find_float_arg(argc, argv, "-hier", .5);
    int cam_index = find_int_arg(argc, argv, "-c", 0);
    int frame_skip = find_int_arg(argc, argv, "-s", 0);//跳帧 5 or more?
    if(argc < 4){
    ...
    if(gpu_list){
        ...
        gpus = calloc(ngpus, sizeof(int));//为ngpus个gpu分配ngpus*4这么多的内存空间
        for(i = 0; i < ngpus; ++i){
            gpus[i] = atoi(gpu_list);//字符串转换成整数型
            gpu_list = strchr(gpu_list, ',')+1;//查找gpu_list字符串中第一次出现逗号的位置
        }
    } else {
        gpu = gpu_index;
        gpus = &gpu;
    ...
    char *datacfg = argv[3];
    char *cfg = argv[4];
    char *weights = (argc > 5) ? argv[5] : 0;//可以继续把训练好的权重文件当基础 如：darknet19_448.conv.23
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh);
        ...
        list *options = read_data_cfg(datacfg);
        int classes = option_find_int(options, "classes", 20);//是否需要修改？
        char *name_list = option_find_str(options, "names", "data/names.list");
        char **names = get_labels(name_list);
